...
// Socket instance created outside component to avoid recreation on renders
...
const ChatBox = () => {
  // State for storing different types of messages
  ...
  // Get current messages based on active chat context
  const getCurrentMessages = () => {
    if (chatMode === "room") {
      return roomMessages[room] || [];
    } else {
      // For private messages, create a unique ID to represent the conversation
      const chatId = getPrivateChatId(userId, recipientId);
      return privateMessages[chatId] || [];
    }
  };

  // Helper to create a consistent chat ID for private messages between two users
  ...
  // Setup socket listeners separately from room/chat mode changes
  useEffect(() => {
    if (!userId) return;
    ...
    socket.on("roomMessage", (message) => {
      // Only update room messages for the specific room
    ...
    socket.on("privateMessage", (message) => {
      const { userId: senderId, recipientId: receiverId } = message;
    ...
    // socket.on("userTyping", ({ userId: typingUserId, room: typingRoom, chatMode: typingMode, recipientId: typingRecipient }) => {
    //   if (typingUserId !== userId) {
    //     // Only show typing indicators for the current context
    //     if ((typingMode === "room" && chatMode === "room" && typingRoom === room) || 
    //         (typingMode === "private" && chatMode === "private" && 
    //         (typingUserId === recipientId || typingRecipient === userId))) {
  ...
  if (typingUserId !== userId) {
    if ((typingMode === "room" && chatMode === "room" && typingRoom === room) || 
      ...
      typingTimeouts.current[typingUserId] = setTimeout(() => {
        setTypingUsers((prev) => ({ ...prev, [typingUserId]: false }));
        delete typingTimeouts.current[typingUserId];
      }, 1500);
    }
  }
});
  ...
  }, [room, chatMode, userId]);

  // Scroll to bottom when current messages update
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    // Add room or recipient based on chat mode
    if (chatMode === "room") {
      message.room = room;
    } else {
    ...
    // Send the message via socket based on chat mode
    if (chatMode === "room") {
      socket.emit("sendMessage", { message, room });
      
      // Update local state for room messages
      setRoomMessages(prev => ({
        ...prev,
      ...
      // Update local state for private messages
      const chatId = getPrivateChatId(userId, recipientId);
      setPrivateMessages(prev => ({
        ...prev,
        [chatId]: [...(prev[chatId] || []), message]
  ...
  const handleChatModeChange = (event, newValue) => {
    if (chatMode !== newValue) {
      // Leave the current room if changing from room mode
      if (chatMode === "room") {
        socket.emit("leaveRoom", room);
      }
      ...
      // Join the selected room if changing to room mode
      if (newValue === "room") {
        socket.emit("joinRoom", room);
      }
    }
